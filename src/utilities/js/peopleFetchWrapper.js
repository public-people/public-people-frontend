  ...
  return get(url).then(data => {
    ...
    return Promise.all(
      data.results.map(datum => {
        ...
        // Now, we request the data for the relevant organisation. The base url is stored in config for easy of access.
        ...
        return get(
          ...
          datum.current_postion === null
            ? (datum.current_postion = datum.memberships[latestPositionIndex])
            : (datum.latest_postion = datum.memberships[latestPositionIndex]);
          return datum;
        });
      })
    ).then(res => {
