...
function Outer({ children: rawChildren, value, modify }) {
  const children = modify ? addPropsToChildren(rawChildren, { value }) : rawChildren;
...
test('Modification applied', () => expect(renderToString(Mod)).toEqual(modResult));
test('Modification not applied', () => expect(renderToString(NoMod)).toEqual(noModResult));
