...
test("Promise cancelled is false", () =>
  expect(token.token.cancelled).toBe(false));
test("Promise executes correctly", () =>
  expect(token.request).resolves.toBe("success"));
test("Cancel fetch has reason inside", () =>
  expect(tokenForCancel.token.cancelled).toBe(
    "Cancelled due to special test reason"
  ));
